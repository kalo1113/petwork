 ...
 * 更新待入账金额（管理员/系统场景）
 * @param {Number} userId 用户ID
 * @param {Number} pendingAmount 待入账金额
...
/**
 * 关联宠物图片（更新photo字段）
 * @param {Object} data 包含petId和photo的对象
 * @returns {Promise} 更新结果
 */
...
/**
 * 新增保险产品
 * @param {Object} insuranceData 保险产品信息
 * @returns {Promise} 新增结果
 */
 ...
 * @param {String} insuranceName 保险名称（可选）
 * @param {Number} petType 宠物类型（可选）
 ...
 * 更新保险产品状态
 * @param {Number} insuranceId 保险ID
 * @param {Number} status 新状态（0=下架 1=上架）
 * @returns {Promise} 更新结果
 */
...
export const getInsuranceImgUrl = (imgPath) => {
  ...
  if (imgPath.startsWith('http://') || imgPath.startsWith('https://')) {
    return `${imgPath}?t=${new Date().getTime()}`;
  }
 ...
 * 更新宠物保险订单状态
 * @param {Number} orderId 订单ID
 * @param {Number} status 新状态（0=已支付 1=已生效 2=已取消）
 * @returns {Promise} 更新结果
 */
...
/**
 * 根据保险订单ID查询用户保险权益
 * @param {Number} insuranceOrderId 保险订单ID
...
/**
 * 更新剩余保额
 * @param {Number} benefitId 权益ID
 * @param {Number} amount 剩余保额
 * @returns {Promise} 更新结果
 */
 ...
 * 更新剩余赠送服务
 * @param {Number} benefitId 权益ID
 * @param {String} serviceJson 服务JSON字符串
 * @returns {Promise} 更新结果
 */
